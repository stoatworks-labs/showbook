...
//! in the same spelling the inspector uses (`screens[scr:1].layers[layer:2].label`)
...
mod tests {
    ...
    fn keys_arrays_by_id() {
        let a = json!({"screens": [{"id": "scr:1", "label": "Main"}, {"id": "scr:2", "label": "Side"}]});
        let b = json!({"screens": [{"id": "scr:1", "label": "Main wall"}, {"id": "scr:3", "label": "New"}]});
        let d = diff(&a, &b);
        let paths: Vec<_> = d.iter().map(|c| (c.kind.clone(), c.path.clone())).collect();
        assert!(paths.contains(&(ChangeKind::Changed, "screens[scr:1].label".into())));
        assert!(paths.contains(&(ChangeKind::Removed, "screens[scr:2]".into())));
        assert!(paths.contains(&(ChangeKind::Added, "screens[scr:3]".into())));
        assert_eq!(d.len(), 3);
    }
