...
/// ureq 3 stops reading a body at 10 MB unless told otherwise, and a show
/// archive is comfortably larger than that. Nothing a library holds is a
/// gigabyte, so that is where the limit sits.
...
pub struct DropboxProvider {
    pub token: String,
    /// Folder inside the app's space or the user's Dropbox, e.g. `/Showbook`.
    pub root: String,
}
...
impl SyncProvider for GoogleDriveProvider {
    ...
    fn upload(&mut self, path: &str, bytes: &[u8], modified: u64) -> Result<()> {
        ...
        if let Some(id) = existing.first().and_then(|f| f.get("id")).and_then(Value::as_str) {
            ...
            let boundary = "showbook-drive-boundary";
...
pub struct OneDriveProvider {
    pub token: String,
    /// Folder path under the drive root, e.g. `Showbook`.
    pub root: String,
}
...
mod tests {
    ...
    fn folder_mirror_both_ways() {
        let base = std::env::temp_dir().join(format!("showbook-sync-{}", uuid::Uuid::new_v4()));
