...
const tauriApi = {
  ...
  devicePull: (dev: DeviceRef, opts: { vendorFile: boolean; deepCapture: boolean; intoShow?: string; name?: string }) =>
    invoke<ImportResult>('device_pull', { dev, opts }),
  devicePush: (dev: DeviceRef, id: string, opts: { vendorFile: boolean; modules: string[]; labels: boolean }) =>
    invoke<{ ok: boolean; log: string[] }>('device_push', { dev, id, opts }),
  deviceRecall: (dev: DeviceRef, req: { kind: string; number: number; screen?: string; program: boolean }) =>
    invoke<void>('device_recall', { dev, req }),
  deviceTake: (dev: DeviceRef, screens: string[]) => invoke<void>('device_take', { dev, screens }),

  convertShow: (id: string, target: Platform, model: string, save: boolean) =>
    invoke<{ show: Show; notes: Note[]; saved: boolean }>('convert_show', { id, target, model, save }),
  capabilities: (platform: Platform, model: string) => invoke<Capabilities>('capabilities', { platform, model }),

  companionExport: (id: string, opts: CompanionExportOptions, path: string) =>
    invoke<{ path: string; type: string; pages: number }>('companion_export', { id, opts, path }),
  ...
  oauthBegin: (provider: string, clientId: string, clientSecret?: string) =>
    invoke<string>('oauth_begin', { provider, clientId, clientSecret: clientSecret || null }),
