...
| **Inspect** | Chassis with every connector and what is on it; inputs, outputs and sources; each screen drawn to scale with its outputs and layers; presets per screen; master presets; layer memories (Event Master user keys); multiviewer layouts; cues. |
...
| **Document** | A PDF (A4 or Letter) or a self-contained web page, in five themes: cover with the production details, the chassis as a frame map, the signal flow, patch tables, one page per screen and per preset drawn to scale, the preset matrix, layer memories with their looks drawn to scale, cues on a time line, multiviewer layouts, a glossary of what the settings mean, the import notes, the version history. |
...
| **Convert** | Event Master ↔ LivePremier (and descriptors for Midra 4K, Alta 4K, LiveCore, PDS-4K, PixelHue): the show is re-keyed in the target's own spelling and held against its capacity. Multi-screen presets become memories plus a master memory and back; user keys and layer memories are carried across as the one idea they are; auxiliary memories are counted against the aux bank where the target keeps one; multiviewer layouts past the live one become multiviewer memories. Every dropped feature is named. |
